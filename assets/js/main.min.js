...
async fetchStats(){try{const clonesResponse=await fetch(`https://maculinx.altervista.org/github/tickzero/clones_public.php?t=${new Date().getTime()}`,);let clones=0;if(clonesResponse.ok){const clonesData=await clonesResponse.json();clones=clonesData.count||0;}
