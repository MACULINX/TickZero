...
class GitHubStats {
    constructor() {
        this.cloneCountEl = document.getElementById('clone-count') || document.getElementById('download-count');
    ...
    async fetchStats() {
        try {
            // Fetch local JSON file generated by GitHub Action
            // Add timestamp query param to prevent caching
            const response = await fetch(`assets/data/stats.json?t=${new Date().getTime()}`);
            ...
            // Validate data
            if (typeof data.clones !== 'number' || typeof data.stars !== 'number') {
                throw new Error('Invalid stats data format');
            }

            // Update UI
            if (this.starCountEl) {
                this.updateStat(this.starCountEl, data.stars);
            }

            if (this.cloneCountEl) {
                // If clones is 0 (due to missing token/error in workflow), show fallback or handled value
                // For now we just show it. 
                this.updateStat(this.cloneCountEl, data.clones);
            }
